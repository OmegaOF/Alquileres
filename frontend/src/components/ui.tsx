...
export function Field({ label, children, required = false }: { label: string; children: ReactNode; required?: boolean }) { return <div className="field"><label>{label}{required && <span className="required-dot" title="Campo obligatorio" aria-label="Campo obligatorio" />}</label>{children}</div>; }
export function FormInput({ label, value, onChange, type, required = false }: { label: string; value: any; onChange: (v: string) => void; type?: string; required?: boolean }) { const inputType = type ?? (label.startsWith("fecha_") ? "date" : "text"); return <Field label={formatLabel(label)} required={required}><input type={inputType} value={value ?? ""} placeholder={formatLabel(label)} onChange={(e) => onChange(e.target.value)} /></Field>; }
export function FormSelect({ label, value, onChange, options, required = false }: { label: string; value: any; onChange: (v: string) => void; options: { label: string; value: string }[]; required?: boolean }) { return <Field label={formatLabel(label)} required={required}><select value={value ?? ""} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field>; }
