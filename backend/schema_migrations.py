...
def ensure_runtime_schema(engine):
    ...
    with engine.begin() as conn:
        if "servicios_mensuales" in insp.get_table_names():
            ...
            conn.execute(text("UPDATE servicios_mensuales SET estado = 'activo' WHERE estado IS NULL"))
        ...
        if "egresos_casa" in insp.get_table_names():
            ...
            conn.execute(text("UPDATE egresos_casa SET estado = 'activo' WHERE estado IS NULL"))
            if "fecha_anulacion" not in cols:
