...
def resumen_periodo(db: Session, id_periodo: int):
    ...
    recuperado = sum(Decimal(d.monto) for c in cobros for d in c.detalles if d.tipo_concepto == "servicio")
    ...
    return {
        ...
        "monto_recuperado_inquilinos": float(recuperado),
        "costo_neto_propietario": float(total_egresos - recuperado),
