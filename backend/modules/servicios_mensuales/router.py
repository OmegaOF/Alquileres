...
def create_item(payload: schemas.ServicioMensualCreate, db: Session = Depends(get_db), _=Depends(get_current_user)): return service.create_item(db,payload)
...
def update_item(item_id:int, payload: schemas.ServicioMensualUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
 item=service.get_item(db,item_id)
 if not item: raise HTTPException(404,"No encontrado")
