...
def create_item(db:Session,payload): d=payload.model_dump(exclude_unset=True); _val(db,d); i=Pago(**d); db.add(i); db.commit(); db.refresh(i); recalcular_cobro(db,i.id_cobro); return i
